...
| [Gandr](https://gandr.ai/) | TTS API for voice agents. First audio byte in 146 ms over the open internet (116 ms p50 server-side), WER below the human baseline on the same scorer, one voice in 23 languages, watermark on every render. Python/JS SDKs, LiveKit plugin, MCP server. | 语音 Agent 专向 TTS API，低延迟，23 种语言，带水印 |
