...
| [Gandr](https://gandr.ai/) | TTS API for voice agents. 116 ms first audio byte (p50), WER below the human baseline on the same scorer, watermark on every render, automatic failover. Python/JS SDKs, LiveKit plugin, MCP server. | 语音 Agent 专向 TTS API，低延迟，带水印 |
